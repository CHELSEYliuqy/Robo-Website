MAXSER_weight_no_short <- function(X, F=NULL, screening=FALSE, Num.Screen=50, sigma, K=10, given.picks=NULL, given.theta=NULL, risk.adj=FALSE, lev.adj=FALSE)
  ...
  ###Special notice: if F is not specified (default setting), MAXSER portfolio weight without factor investing ("Scenario I" in the paper) will be computed. As long as F is specified, factor investing will be included in MAXSER strategy.
{
  ...
  if(is.null(F)==TRUE){
    ...
    lars.est <- lasso.ineq(X, Y, C.full=matrix(0, nrow=dim(X)[2]-1, ncol=dim(X)[2]), b=rep(0,(dim(X)[2]-1)), normalize=FALSE, intercept=FALSE, step=0.1)  # , l.min=-2, l.max=0.77, step=0.01
    lars.est.cons <- colSums(lars.est$coefs >= 0)
    lars.est$lambda[which(lars.est.cons < dim(X)[2])] <- Inf
    
    num.solu <- dim(t(lars.est$coefs))[1]
  ...
  if(is.null(F)==FALSE){
    ...
    cov.f <- cov(F)
    w_fac <- sigma/sqrt((t(matrix(mu.f))%*%solve(cov.f)%*%matrix(mu.f))[1])*solve(cov.f)%*%mu.f
    ...
    if(theta.u > 0){
      ...
      #constraint <- -(sqrt(theta.f / theta.u) * w_fac + lower.b * sqrt(theta / theta.u) / sigma)[1]
      lars.est <- lasso.ineq(E, Y, C.full=matrix(0, nrow=dim(E)[2]-1, ncol=dim(E)[2]), b=rep(0,(dim(E)[2]-1)), normalize=FALSE, intercept=FALSE, step=0.1)  # , l.min=-2, l.max=0.77, step=0.01
      lars.est.cons <- colSums(lars.est$coefs >= 0)
      #lars.est.cons_1 <- colSums(lars.est$coefs == 0)
      ...
      out <- foreach(i=seq(K), .combine='cbind', .packages='PACLasso') %dopar%{
        ...
        fit <- lasso.ineq(E[ - omit,,drop=FALSE  ], Y[ - omit], C.full=matrix(0, nrow=dim(E)[2]-1, ncol=dim(E)[2]), b=rep(0,(dim(E)[2]-1)), normalize=FALSE, intercept=FALSE, step=0.1)  # , l.min=-2, l.max=0.77, step=0.01
        cons.result <- colSums(fit$coefs >= 0)
        #cons.result_1 <- colSums(fit$coefs == 0)
        ...
        # frac <- rowSums(abs(t(fit$coefs)))/sum(abs(t(fit$coefs)[num.solu,]))
        lambda.temp <- fit$lambda[which.min(residmat[,i])]
        CV.risk.temp <- temp_risk[which.min(residmat[,i])]
        CV.lev.temp <- sigma / CV.risk.temp
        c(lambda.temp, CV.risk.temp, CV.lev.temp)
      ...
      #   omit <- all.folds[[i]]
      #   # fit <- lars(E[ - omit,,drop=FALSE  ], Y[ - omit], type="lar",normalize=FALSE,intercept=FALSE)
      #   fit <- lars.ineq(E[ - omit,,drop=FALSE  ], Y[ - omit], C.full=matrix(0, nrow=dim(E)[2]-1, ncol=dim(E)[2]), b=rep(0,(dim(E)[2]-1)), normalize=FALSE, intercept=FALSE, l.min=-2, l.max=0.77, step=0.01)
      #   cons.result <- colSums(fit$coefs >= 0)
      #   #cons.result_1 <- colSums(fit$coefs == 0)
      #   # w.ucv<- (sqrt(x.hat)*sigma*coef(fit))
      #   w.ucv <- (sqrt(x.hat)*sigma*t(fit$coefs))
      ...
      #   # frac<-rowSums(abs(coef(fit)))/sum(abs(coef(fit)[num.solu,]))
      #   # frac <- rowSums(abs(t(fit$coefs)))/sum(abs(t(fit$coefs)[num.solu,]))
      #   lambda[i] <- fit$lambda[which.min(residmat[,i])]
      #   CV.risk[i] <- temp_risk[which.min(residmat[,i])]
      #   CV.lev[i] <- sigma / CV.risk[i]
      #   # frac<-rowSums(abs(coef(fit)))/sum(abs(coef(fit)[dim(coef(fit))[1],]))
      #   # zeta[i]<-frac[which.min( abs(sigma-apply(outfit, 2, sd)) )]
      # }
