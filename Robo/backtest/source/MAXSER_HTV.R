  ...
  ##Note: this version applies KZ to normailzed idiosyncratic returns to estimate theta_u
{
  ...
  if(factor.invest==TRUE){
   ...
   if(theta.u==0){
    ...
    w.F<-sigma/sqrt(theta.all)*solve(Sigma.f)%*%mu.f-t(Beta)%*%w.A
    ...
    outlist<-list(prob.gamma.star=prob.gamma.star, probs.star=probs.star, thresh=thresh, MSE.list=MSE.list,picks=picks, w.maxser=w.maxser, w.F=w.F, w.A=w.A, w.mhtv=w.mhtv, tv_scalar=tv_scalar, x.hat=x.hat,theta.re=theta.re, theta.all=theta.all, theta.f=theta.f, theta.u=theta.u, Beta=Beta, Alpha=Alpha, Sigma.f=Sigma.f, zeta.star=zeta.star, mu.f=mu.f, gamma=gamma, E_gamma2=E_gamma2, normalize_factors=normalize_factors, gamma.predicted=gamma.predicted, gamma.predict.method=gamma.predict.method, useKZ=useKZ)
  }
