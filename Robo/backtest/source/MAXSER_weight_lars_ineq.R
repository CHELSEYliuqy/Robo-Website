MAXSER_weight<-function(X, F=NULL, screening=FALSE, Num.Screen=50, sigma, K=10, given.picks=NULL, given.theta = NULL)
  ...
  ###Special notice: if F is not specified (default setting), MAXSER portfolio weight without factor investing ("Scenario I" in the paper) will be computed. As long as F is specified, factor investing will be included in MAXSER strategy.
{
  ...
  if(is.null(F)==FALSE){
    ...
    if(theta.u>0){
      ...
      lars.est <- lars.ineq(E, Y, C.full=as.matrix(t(cbind(-Beta, rep(0, length(Beta))))), b=c(constraint, 0), normalize=FALSE, intercept=FALSE, l.min=-2, l.max=0.77, step=0.01)
      lars.est.cons <- t(lars.est$coefs) %*% -Beta
      lars.est$lambda[which(lars.est.cons < constraint)] <- Inf
      ...
      for(i in seq(K)) {
        ...
        fit <- lars.ineq(E[ - omit,,drop=FALSE  ], Y[ - omit], C.full=as.matrix(t(cbind(Beta, rep(0, length(Beta))))), b=c(constraint, 0), normalize=FALSE, intercept=FALSE, l.min=-2, l.max=0.77, step=0.01)
        cons.result <- t(fit$coefs) %*% -Beta
        ...
        if(length(omit)==1) outfit<-matrix(outfit,nrow=1)
        residmat[, i] <- abs(sigma-apply(outfit, 2, sd))
        residmat[, i][which(cons.result < constraint | cons.result == 0)] <- Inf
    ...
    outlist<-list(lars.est=lars.est, lambda.star = lambda.star, l1norm = l1norm,
                  ...
                  rmse = rmse, r_c = r_c, w.fcv.record=w.fcv.record)
  }
  
  return(outlist)
}

# write.csv(E, file="E.csv",quote=F,row.names = F)
# write.csv(fac.wt, file="Y.csv",quote=F,row.names = F)
