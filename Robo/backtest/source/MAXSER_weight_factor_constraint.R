...
MAXSER_weight_factor_constraint <- function(X, F=NULL, screening=FALSE, Num.Screen=50, sigma, K=10, given.picks=NULL, thres=NULL, given.theta=NULL, risk.adj=FALSE, lev.adj=FALSE, est='theta-all')
{
  ...
  if(is.null(F) == FALSE){
    ...
    w_fac <- 1 / sqrt(theta.f) * solve(Sigma.f) %*% mu.f
    ...
    if(theta.u > 0){
      ...
      lars.est <- lasso.ineq(E, Y, C.full=as.matrix(t(cbind(-Beta, rep(0, length(Beta))))), b=c(constraint, 0), normalize=FALSE, intercept=FALSE, step=0.05)
      ...
      out <- foreach(i=seq(K), .combine='cbind', .packages='PACLasso') %dopar%{
        ...
        fit <- lasso.ineq(E[ - omit,,drop=FALSE  ], Y[ - omit], C.full=as.matrix(t(cbind(-Beta, rep(0, length(Beta))))), b=c(constraint, 0), normalize=FALSE, intercept=FALSE, step=0.05)
        ...
        temp_risk <- apply(outfit, 2, sd)
        residmat[, i] <- abs(sigma - temp_risk)
        residmat[, i][which(cons.result < constraint)] <- Inf
