    ...
    if(theta.u>0){
      ...
      out <- foreach(i=seq(K), .combine='cbind', .packages='PACLasso') %do%{
        ...
        for(s in 1:(dim(w.ucv)[1])) 
          w.fcv[s,] <- c(sigma/sqrt(theta.all)*solve(Sigma.f)%*%mu.f-t(Beta)%*%matrix(w.ucv[s,]))
        ...
        residmat[, i][which(cons.result < dim(U_c)[2])] <- Inf
