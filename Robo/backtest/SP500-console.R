#' --------------------------------Parameters List--------------------------------- # nolint
#' @param market select the backtesting market, eg. SSE50, HS300, CSI500, HSI, SP500, etc # nolint
...
btp <- tibble(
  ...
  factor_used = 'ETF',
  factor_name = list(c('SPY')),

  sigma = NA,
  sigma_annual = as.numeric(sigma_target),

  stk_screen_mtd = 'max_mkt_cap',
  stk_pool_num = 100,
  stk_pool = NA,
  pure_random = FALSE,
  theta_loc = 950,
